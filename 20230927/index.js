...
const filterRestaurant = (restaurants, veganFriendly, maxPrice, maxDistance) =>
  restaurants
    .filter(([_, _rating, isVeganFriendly, price, distance]) =>
      isVeganFriendly === veganFriendly && price <= maxPrice && distance <= maxDistance
    )
    .sort((a, b) => b[1] ===a[1] ? b[0] - a[0] : b[1] - a[1])
    .map(([name]) => name);

const restaurants = [[1,4,1,40,10],[2,8,0,50,5],[3,8,1,30,4],[4,10,0,10,3],[5,1,1,15,1]],veganFriendly = 0, maxPrice = 50, maxDistance = 10
